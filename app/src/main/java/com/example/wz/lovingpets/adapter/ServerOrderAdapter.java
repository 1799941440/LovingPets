...
public class ServerOrderAdapter extends RecyclerView.Adapter<ServerOrderAdapter.SOHolder> {
    ...
    public void onBindViewHolder(@NonNull SOHolder holder, int position) {
        ...
        setBTs(holder.bt_left,holder.bt_right,data.getState(),data.getId());
    }


    private void setBTs(final Button bt_left, final Button bt_right, byte state, final int id) {
        if(state == 0){
            ...
            bt_left.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    EventBusUtils.sendEvent(new Event<Integer>(EventCodes.CANCEL_SERVER_ORDER,id));
                }
            });
        }else if(state == 1){
            ...
            bt_left.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    EventBusUtils.sendEvent(new Event<Integer>(EventCodes.DELAY_SERVER_ORDER,id));
                }
            });
        }else if(state == 2){
            ...
            bt_left.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    EventBusUtils.sendEvent(new Event<Integer>(EventCodes.PAY_SERVER_ORDER,id));
                }
            });
            ...
            bt_left.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    EventBusUtils.sendEvent(new Event<Integer>(EventCodes.DEL_SERVER_ORDER,id));
                }
            });
        }else if(state == 4){
            bt_left.setText("删除订单");
